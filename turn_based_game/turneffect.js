...
(() => {
    ...
    register('poison_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'poison');
    });

    // 冻伤伤害
    register('frostbite_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'frostbite');
    });

    // 烧伤伤害
    register('burn_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'burn');
    });

    // 焚烬伤害
    register('immolate_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'immolate');
    });
    ...
    register('silence_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'silence');
    });

    // 流血伤害
    register('bleed_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'bleed');
    });

    // 寄生吸取
    register('parasite_tick', (game, { char, opponent, effect }) => {
        const dmg = window.DamageEffect?.statusDamage(game, char, 'parasite');
        if (dmg > 0) {
            window.HealEffect?.heal(game, opponent, dmg, "寄生");
        }
    });

    // 烈焰诅咒
    register('curse_fire_tick', (game, { char, opponent, effect }) => {
        window.DamageEffect?.statusDamage(game, char, 'curse_fire');
    });
    ...
    register('bind_expire', (game, { char, opponent, effect }) => {
        ...
        window.DamageEffect?.fixed(game, char, dmg, "束缚结束");
    });
    ...
    register('water_curse_tick', (game, { char, opponent, effect }) => {
        ...
        window.DamageEffect?.percent(game, char, pct, `水厄伤害(层数:${stacks})`);
    });
    ...
    register('regen_tick', (game, { char, opponent, effect }) => {
        if (!window.StatusEffect?.isControlled(char)) {
            window.HealEffect?.regenPercent(game, char, 1/8, "再生");
        }
    });
    ...
    register('fire_core_tick', (game, { char, opponent, effect }) => {
        ...
        window.HealEffect?.heal(game, char, amount, "火焰精核");
        window.DamageEffect?.fixed(game, opponent, amount, "火焰精核");
    });
    ...
    register('absorb_hp_skill_tick', (game, { char, opponent, effect }) => {
        ...
        if (!game.isStarSovereign(opponent)) {
            window.DamageEffect?.drainPercent(game, char, opponent, 1/ratio, "吸血");
        } else {
    ...
    register('absorb_loop_tick', (game, { char, opponent, effect }) => {
        ...
        window.DamageEffect?.drain(game, char, opponent, absorbAmt, "无始源光");
    });
    ...
    register('eternal_fire_tick', (game, { char, opponent, effect }) => {
        ...
        if (Math.random() * 100 < chance) {
            ...
            window.DamageEffect?.fixed(game, opponent, cut, "火种永存");
        }
    });
    ...
    register('fixed_damage_loop_tick', (game, { char, opponent, effect }) => {
        ...
        window.DamageEffect?.fixed(game, opponent, dmg, effect.name || "固定伤害");
    });
    ...
    register('incalos_skill_trap_tick', (game, { char, opponent, effect }) => {
        if (char.buffs.usedSkillThisTurn) {
            ...
            window.DamageEffect?.fixed(game, char, dmg, "光之惩戒");
        }
    });
    ...
    register('incalos_hit_trap_tick', (game, { char, opponent, effect }) => {
        if (char.buffs.tookDamageThisTurn) {
            ...
            window.DamageEffect?.drain(game, char, opponent, absorbAmt, "光之惩戒");
        }
    });

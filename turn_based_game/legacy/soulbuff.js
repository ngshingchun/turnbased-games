...
(() => {
    ...
    function routeToDamageEffect(game, target, damageType, amount, options = {}) {
        if (window.HpEffect) {
            switch (damageType) {
                case 'fixed':
                    return window.HpEffect.fixed(game, target, amount, options.source || '魂印');
                case 'percent':
                    return window.HpEffect.percent(game, target, amount, options.source || '魂印');
                case 'drain':
                    return window.HpEffect.drain(game, options.attacker, target, amount);
            }
        }
        return 0;
    }
    ...
    function routeToHealEffect(game, target, amount, options = {}) {
        if (window.HpEffect?.heal) {
            return window.HpEffect.heal(game, target, amount, options.source || '魂印');
        }
        return 0;
    }
    ...
    window.SoulEffect = window.SoulBuff;
})();
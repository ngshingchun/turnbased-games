...
(() => {
    ...
    const TEAM_B = {
        ...
        init(spirits = []) {
            if (!window.createTeamData) {
                console.error('[TEAM_B] TEAM system not loaded');
                return null;
            }
            
            this._data = window.createTeamData('B', spirits);
            this._initialized = true;
            
            // 同步到 TEAM 系统
            if (window.TEAM) {
                window.TEAM._teamB = this._data;
            }
            
            console.log(`[TEAM_B] Initialized with ${spirits.filter(s => s).length} spirits`);
            return this._data;
        },
         ...
         * AI 难度设置
         */
        aiDifficulty: 'normal',  // 'easy', 'normal', 'hard', 'extreme'
         ...
         * (简单实现，可扩展)
         */
        getAIAction(opponentTeam) {
            const current = this.current();
            if (!current || !current.isAlive) {
                // 需要切换
                const available = this.getAvailableSwitches();
                if (available.length > 0) {
                    return { type: 'switch', index: available[0] };
                }
                return { type: 'none' };
            }
            
            // 简单 AI: 随机选择可用技能
            const usableSkills = current.skills
                .map((s, i) => ({ skill: s, index: i }))
                .filter(({ skill }) => skill.currentPp > 0);
            ...
            switch (this.aiDifficulty) {
                case 'easy':
                    // 随机选择
                    const random = usableSkills[Math.floor(Math.random() * usableSkills.length)];
                    return { type: 'skill', index: random.index };
                    
                case 'hard':
                case 'extreme':
                    // 优先高威力技能
                    usableSkills.sort((a, b) => (b.skill.power || 0) - (a.skill.power || 0));
                    return { type: 'skill', index: usableSkills[0].index };
                ...
                default:
                    ...
                    const rand = usableSkills[Math.floor(Math.random() * usableSkills.length)];
                    return { type: 'skill', index: rand.index };
            }
        },
        ...
        import(data) {
            // 需要配合精灵注册表重建完整数据
            // 这里只是框架，实际实现需要更多逻辑
            console.log('[TEAM_B] Import not fully implemented');
            return false;
        }
    };
    ...
    console.log('[TEAM_B] Module loaded');
})();
